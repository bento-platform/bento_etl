...
from bento_etl.transformers.plugins.phenopackets.phenopackets_transformer import PhenopacketsTransformer
from bento_etl.transformers.plugins.experiments.experiments_transformer import ExperimentsTransformer
...
def get_transformer(job: Job, logger: LoggerDependency,  config: ConfigDependency):
    ...
    if job.transformer.type == "phenopackets":
        return PhenopacketsTransformer(
            logger, config
        )
    elif job.transformer.type == "experiments":
        return ExperimentsTransformer(
            logger, config
        )
    else:
