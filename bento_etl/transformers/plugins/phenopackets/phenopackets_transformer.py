...
class PhenopacketsTransformer(BaseTransformer):
    def __init__(self, logger: Logger, config: Config):
        ...
        self.mappings = (
            json.load(open(config.phenopackets_mappings_path))
            if hasattr(config, "phenopackets_mappings_path")
            else {}
        )
        self.ontology_mappings = (
            json.load(open(config.phenopackets_ontology_mappings_path))
            if hasattr(config, "phenopackets_ontology_mappings_path")
            else {}
        )
    ...
    def transform(self, raw: Any) -> list[dict]:
        ...
        return perform_transformation(
            raw, schema, self.mappings, self.ontology_mappings, self.logger
        )
