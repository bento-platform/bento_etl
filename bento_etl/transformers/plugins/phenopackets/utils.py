...
def associate_referenced_rows(participant_entities, participant_ids, input_data, group_field):
    for p_id in participant_ids:
        for table in input_data:
            for row in input_data[table]:
                if row.get(group_field) is None and any(p_id in str(v) for v in row.values()):
                    participant_entities[p_id][table].append(row)

def process_non_array_mappings(phenopacket, participant_entities, non_array_mappings, ontology_mappings):
...
def process_array_mappings(phenopacket, participant_entities, array_sources, ontology_mappings, array_item_schemas, per_field_arrays):
...
def build_phenopacket(p_id, participant_entities, leaf_mappings, non_array_mappings, array_sources, ontology_mappings, array_item_schemas, per_field_arrays, array_dedup_keys):
    phenopacket = {"id": p_id.replace(' ', '_'), "subject": {"id": p_id.replace(' ', '_')}}
    phenopacket["meta_data"] = META_DATA.copy()
    process_non_array_mappings(phenopacket, participant_entities, non_array_mappings, ontology_mappings)
    process_array_mappings(phenopacket, participant_entities, array_sources, ontology_mappings, array_item_schemas, per_field_arrays)
    for array_name in array_dedup_keys:
        if array_name in phenopacket:
            phenopacket[array_name] = merge_items(phenopacket[array_name], array_dedup_keys[array_name])
    prune_empty(phenopacket)
    return phenopacket

def perform_transformation(raw: Any, schema: dict, mappings: dict, ontology_mappings: dict, logger) -> list[dict]:
    array_item_schemas = {}
    ...
    participant_entities, participant_ids = group_by_participant(input_data, group_field)
    associate_referenced_rows(participant_entities, participant_ids, input_data, group_field)
    ...
    for p_id in sorted(participant_entities):
        phenopacket = build_phenopacket(p_id, participant_entities[p_id], leaf_mappings, non_array_mappings, array_sources, ontology_mappings, array_item_schemas, per_field_arrays, array_dedup_keys)
        try:
